...
cinep_cov <- cinep_cov %>%
  filter(year %in% years) %>%
  dplyr::select(ID_MUN, pop, popch, SocServicespc_b08, Forest_pctg, slope, year) %>%
  group_by(ID_MUN, year) %>%
  # group_by(ID_MUN) %>%
  summarize(across(c(pop, popch, SocServicespc_b08, Forest_pctg, slope), ~mean(., na.action = NULL, na.rm = TRUE)),
            year = year[1],
            .groups = "keep") %>%
  # summarise_each( ~ mean(., na.action = NULL, na.rm = TRUE)) %>%
...
#-----------------------------------------------------------------------------#
# ADDED: 2021-02-23
# Code to produce a panel spatial data frame for PA R&R
years <- 2002:2009

col2 <- st_make_valid(colombia)
...
for(y in years){
  dtmp <- event_farc %>%
    filter(year == y) %>%
    dplyr::select(-year)
  ags <- aggregate(dtmp, col2, FUN = function(x){sum(x, na.rm = T)})
  ags <- ags %>%
    mutate(year   = y,
           ID_Mun = col2$ID_Mun) %>%
    replace(is.na(.), 0) %>%
  ...
  ags <- left_join(col2, ags, by = "ID_Mun")

  dat <- bind_rows(dat, ags)
}
rm(ags, dtmp, y)

dat <- dat %>% left_join(.,cinep, by = c("ID_Mun", "year")) %>%
  replace(is.na(.), 0)
...
save(dat, file = "Data/colombia_panel.Rdata")
